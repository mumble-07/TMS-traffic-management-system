...
local_file = 'rpiC.jpg'                  # Local file path for the saved image
remote_file = 'rpiC.jpg'                 # Remote path on the FTP server
...
def initialize_database():
    ...
    try:
        ...
        create_table_query = """
        CREATE TABLE IF NOT EXISTS vehicle_detections_rpiC (
...
def insert_detection(cursor, db_connection, class_name, confidence, current_time, x1, y1, x2, y2, vehicle_count):
    ...
    try:
        insert_query = """
            INSERT INTO vehicle_detections_rpiC (vehicle_type, quantity, class_name, confidence, timestamp, x1, y1, x2, y2)
